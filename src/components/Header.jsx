      ...
      {navItems.map(item => (
        <Link
          ...
          style={{
            ...
            fontWeight: pathname === item.path ? 600 : 400, transition: 'color 0.2s'
          }}
    ...
    <>
      <header style={headerStyle}>
        <div className="container" style={{
          ...
          {/* Logo always on left for mobile or desktop when visible */}
          ...
          {/* Desktop Navigation or Spacer */}
          {navLinks}

          {/* Mobile Burger Icon always on right */}
          {isMobile && (
            <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
              {menuOpen ? <X size={24} color={isHome && !isScrolled ? '#2A7E63' : '#2A7E63'} /> : <Menu size={24} color={isHome && !isScrolled ? '#fff' : '#2A7E63'} />}
            </button>
          )}
        </div>
      </header>
